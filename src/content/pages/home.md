---
title: "Home"
description: ""
date: "2024-12-13 16:19:47"
updated: "2026-09-19 20:25:51"
slug: "home-new"
categories: []
tags: []
legacy_url: "https://pepepow.org/"
source_url: "https://pepepow.org/"
status: "draft"
featured: false
migration_review: true
---

> Migration candidate generated from the legacy WordPress export. Review facts, links, software versions, commands, and media before publishing.

![](/media/legacy/2024/12/PEPEPOW-sci.png)
[Join new discord
(since 2024.2.24)](https://discord.gg/sJgDVRkBcq)

#### Running on HooHash V110

## A decentralized blockchain project

Inspired by the iconic Pepe meme, this platform has been created on f**irst layer blockchain**, fostering creativity and global collaboration. The mission is to bridge the gap between people of all languages and skillsets, connecting them through a decentralized network that supports learning, growth, and real-world opportunities.

### Total supply

The total supply is capped at **90** **billion** coins.

### Dual reward system

Both **miners** and **masternode** holders share the block rewards, incentivizing participation and maintaining network stability.

### Current Proof-of-Work

PEPEPOW currently uses **HooHash V110** for Proof-of-Work mining. Current mining tooling supports CPU and GPU participation through the HooHash ecosystem.

![](/media/legacy/2024/02/a-graphy-pepepow.png)
![](/media/legacy/2024/02/P-coin4.png)

### Superblocks

Random blocks with 2-5x higher rewards

### Fast block times

Fast 20 second block times: Quick and affordable transactions

![](/media/legacy/2024/02/PEPEPOW-helmet5.png)

##### DAO (Decentralized Autonomous Organization)

## Community-driven program

###### [PEPEPOW DAO reference (25 Otc 2023, Minus)](/docs/legacy/2023/10/pepepow-dao-op-policy-v1.0.0.pdf)

###### [PEPEPOW DAO structure (17 Jan 2024, Edison)](/docs/legacy/2023/10/pepepow-dao-op-policy-v1.0.0.pdf)

The project is overseen by a team of **community-elected members** dedicated to its development. They are actively involved in seeking, establishing, and maintaining partnerships to expand the network's reach and capabilities.

### Transparency

All data are open for everyone co-governance.

### Voting for debate

Referring to all opinions, and all decisions should be the greatest consensus.

### It is not the strongest of the species that survives, nor the most intelligent; it is the one most adaptable to change.

### - Charles Darwin (1809-1882)

##### English naturalist, geologist, and biologist

#### Bridge for global connections

## Proof-of-work network

###### [PEPEPOW white paper V1.0 (18 Sep 2023, Minus)](/docs/legacy/2023/09/whitepaper-v1.0.1.pdf)

###### [PEPEPOW Story ( 8 Otc 2023, Stevin)](https://github.com/MattF42/PePe-core/blob/master/doc/pepeproofofworkstory.md)

###### [PEPEPOW white paper V2.0 (26 Jan 2025, Edison)](https://docs.google.com/document/d/1FIs7lQo_7tjNhJ9n95_YrvuIGmLlSjmkKqEWIktlU5Q/edit?tab=t.0)

###### [PEPEPOW white paper V2.1 (31 may 2026, Edison)](https://docs.google.com/document/d/1zeSVPZAJx6A5azjBvSGmbDs1xx8lTxoebCtDZuQhv3I/edit)

The PepePow whitepaper outlines a new blockchain project built around the popular Pepe meme. PepePow aims to provide a platform for community collaboration, skill development, and forming global connections. The network is secured through proof-of-work mining and incentivized by both miners and masternodes.

### PEPEPOW's Survival

PEPEPOW encountered a chain disconnection and subsequent death incident. Following this survival, the developer fee was suspended for an extended period.

### Developer Fee Reinstated

Following the vote, the developer fee was reinstated, and the development process is progressing forward.

## PEPEPOW Development Roadmap

Completed
Partially Completed
Not Completed

### 1. API Server

Completed

**Core purpose:** REST backend for wallets, explorers, and external apps. Backbone for fast sync, balance, and transaction queries.

Difficulty: Medium

### 2. ElectrumX Server

Completed

**Core purpose:** Lightweight backend for SPV/light wallets, enabling fast address balance and transaction history queries without full node sync.

Difficulty: Medium-High

### 3. Web Wallet (Non-custodial)

Completed

**Core purpose:** Browser-based wallet for key generation, balance viewing, and sending transactions without a custodian.

Difficulty: Medium

### 4. PepewPay – Payment Link / POS PWA

Not Completed

**Core purpose:** One-page merchant payment system with payment links, QR codes, and live confirmation for in-store and online payments.

Difficulty: Medium

### 5. JS/TS SDK (pepew-js)

Not Completed

**Core purpose:** Unified JavaScript/TypeScript toolkit for key derivation, transaction building/signing, and REST/WebSocket integration.

Difficulty: Medium

### 6. Mobile Wallet (Android / iOS)

Partially Completed

**Core purpose:** Native mobile wallet with fast sync, PIN/biometric lock, QR scanning, and push notifications.

Difficulty: Medium-High

### 7. Telegram Mini Wallet Bot

Completed

**Core purpose:** Social wallet inside Telegram for tipping, micro-payments, and easy onboarding.

Difficulty: Medium-High

### 8. PEPEW Inscriptions & Viewer

Not Completed

**Core purpose:** On-chain creative ecosystem (Ordinals-style) with explorer viewer for inscriptions.

Difficulty: Medium

### 9. Unity Game Kit v1

Not Completed

**Core purpose:** Demo for integrating PEPEPOW into Unity games so players can earn and spend PEPEW.

Difficulty: Medium

### 10. Masternode / Staking Tools & Monitoring

Partially Completed

**Core purpose:** Tools and dashboards for automatic masternode setup, reward tracking, and status alerts.

Difficulty: Medium-High

### 11. Multi-signature Vault Wallet

Not Completed

**Core purpose:** Secure multi-signature treasury for teams or DAOs with approval workflows and optional time-locks.

Difficulty: Medium-High

### 12. Minimal Oracle Gateway

Not Completed

**Core purpose:** Provide signed and verifiable price and event feeds for bridges and on-chain applications.

Difficulty: High

### 13. Liquidity Pool (wPEPEW on EVM or other chains)

Not Completed

**Core purpose:** Deploy wrapped PEPEW and create a USDT/wPEPEW liquidity pool for trading and price discovery.

Difficulty: High

### 14. DEX Aggregator Integration

Not Completed

**Core purpose:** Integrate wPEPEW into major DEX aggregators (e.g., 1inch, Matcha) to expand liquidity and visibility.

Difficulty: High

### 15. Mining Software and Pool Infrastructure

Completed

**Core purpose:** Maintain HooHash-compatible mining tools, pool software, Stratum services, monitoring, reward accounting, and operator documentation.

Difficulty: Medium-High

[![](/media/legacy/2024/02/cc-logo-glyph-pngdark.png)](https://coincodex.com/crypto/pepepow/)
[![](/media/legacy/2024/03/logo_m4p.jpg)](https://mining4people.com/)
[![](/media/legacy/2024/03/logo_concheckup.jpg)](https://coincheckup.com/coins/pepepow/)
[![](/media/legacy/2024/02/logotype-dark-on-light-color-1.png)](https://www.livecoinwatch.com/price/PEPEPOW-PEPEW)
[![](/media/legacy/2024/03/nb_logo_monochrome_white.png)](https://nodehub.io/dashboard/view_coin?coin=pepepow)
[![](/media/legacy/2025/11/pecunia-logo.jpg)](https://pecuniaplatform.io/coin-stats/PEPEPOW-100M)
[![](/media/legacy/2024/02/Y3TI.png)](https://y3ti.uk/audits/PEPEPOW)
[![](/media/legacy/2024/02/nonkyc_logo_exchange_yellow.png)](https://nonkyc.io/asset/pepew)
[![](/media/legacy/2024/02/cropped-blockspot-logo-white.png)](https://blockspot.io/coin/pepepow/)
[![](/media/legacy/2024/03/logo_coinpaprika.jpg)](https://coinpaprika.com/coin/pepew-pepe-pow/)
[![](/media/legacy/2025/11/binance-logo.png)](https://www.binance.com/en/price/pepepow)
[![](/media/legacy/2024/02/Caldera-logo.png)](https://discord.gg/kYeYbWu6Bm)
[![](/media/legacy/2024/02/whattomine.png)](https://whattomine.com/coins/385-pepew-memehash)
[![](/media/legacy/2025/11/MiningPoolStats-logo.jpg)](https://miningpoolstats.stream/pepepow)
[![](/media/legacy/2025/11/mingpowerx-logo-1.png)](https://miningpower.eu/overview)
[![](/media/legacy/2024/03/crypto.com_.jpg)](https://crypto.com/price/pepepow)
[![](/media/legacy/2025/11/NestEx-logo.png)](https://trade.nestex.one/spot/PEPEW)
[![](/media/legacy/2024/03/logo_coinCarp.jpg)](https://www.coincarp.com/currencies/pepepow/)
[![](/media/legacy/2025/02/dex-trade-icon-1.jpg)](https://dex-trade.com/spot/trading/PEPEWUSDT)
[![](/media/legacy/2024/06/coinbase.png)](https://www.coinbase.com/price/pepepow)
