...
def requestable_url(url: str) -> str:
    """Convert a public IRI-style URL into an ASCII URI for urllib/http.client."""
    parts = urlsplit(url)
    try:
        netloc = parts.netloc.encode("idna").decode("ascii")
    except UnicodeError:
        netloc = parts.netloc
    path = quote(parts.path, safe="/%:@+~!USER_AGENT = "pepepow-site-migration/1.0 (+https://pepepow.net)"


def request_once'()*;,=")
    query = quote(parts.query, safe="=&%:@/?+~!(url: str, method: str, timeout: float) -> dict[str, Any]:
    headers = {"User-Agent": USER_AGENT}
    if method == "GET":
        headers["Range"] = "bytes=0-0"
    request = urllib.request.Request(requestable_url(url), headers=headers, method=method)
...
()*;,")
    fragment = quote(parts.fragment, safe="%:@/?+~!USER_AGENT = "pepepow-site-migration/1.0 (+https://pepepow.net)"
